...
class ObjectBase
{
	...
	)
	{
		...
		{
			throw new TypeError(`[${this.constructor.name}] object needs to be of type Object. Current value: ${object === undefined ? "undefined" : JSON.stringify(object)}`);
		}
		...
		for(const strPropertyName in this._schemaDefinition().properties)
		{
			...
			objFinal[strPropertyName] = this._validateItem({value: objFinal[strPropertyName], propertyPath: `${propertyPath}${strPropertyName}`, objPropertyCanBe, errorMessages, objProperty, arrPropertyTypes, applySchemaDefaults, destroyInputReadOnlyProperties, arrUndefinedPropertyNames});
	...
	_validateItem({value, propertyPath, objPropertyCanBe, errorMessages, objProperty, arrPropertyTypes, destroyInputReadOnlyProperties, applySchemaDefaults, bEnumSupported = true, arrUndefinedPropertyNames})
	{
		if(value === null)
		{
		...
		else if(typeof value === "object" && value !== null)
		{
          ...
          Objects = require(`./Objects`);
				...
				const classReference = Objects[value.type];
			...
			else
			{
              ...
              Objects = require(`./Objects`);
						...
						const classReference = Objects[item.type];
		...
		else if(typeof value === "string")
		{
			...
			if(typeof value === "string")
			{
				...
				if(objProperty.hasOwnProperty("maxLength") && Number.isInteger(objProperty.maxLength) && value.length > objProperty.maxLength)
				{
					errorMessages.push(`${propertyPath} value ${value} is longer than the allowed maximum length of ${objProperty.maxLength}`);
				}
				
				if(objProperty.hasOwnProperty("minLength") && Number.isInteger(objProperty.minLength) && value.length < objProperty.minLength)
				{
					errorMessages.push(`${propertyPath} value ${value} is shorter than the required minimum length of ${objProperty.minLength}`);
				}
			}
		}
		...
		)
		{
			...
			else if(value === undefined && objProperty.enum.length === 1)
			{
				value = objProperty.enum[0];
			}
